...
egg_desktop_file_new_from_data_dirs(const char *desktop_file_path,
  ...
  key_file = g_key_file_new();
  if (!g_key_file_load_from_data_dirs(key_file, desktop_file_path, &full_path,
                                      0, error)) {
    g_key_file_free(key_file);
    return NULL;
  }
...
EggDesktopFile *egg_desktop_file_new_from_key_file(GKeyFile *key_file,
  ...
  if (!g_key_file_has_group(key_file, EGG_DESKTOP_FILE_GROUP)) {
    g_set_error(error, EGG_DESKTOP_FILE_ERROR, EGG_DESKTOP_FILE_ERROR_INVALID,
  ...
  if (!strcmp(type, "Application")) {
    ...
    exec = g_key_file_get_string(key_file, EGG_DESKTOP_FILE_GROUP,
    ...
    url = g_key_file_get_string(key_file, EGG_DESKTOP_FILE_GROUP,
  ...
  /* Check the Icon key */
  desktop_file->icon = g_key_file_get_string(key_file, EGG_DESKTOP_FILE_GROUP,
...
gboolean egg_desktop_file_has_key(EggDesktopFile *desktop_file, const char *key,
                                  GError **error) {
  return g_key_file_has_key(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP, key,
                            error);
}

char *egg_desktop_file_get_string(EggDesktopFile *desktop_file, const char *key,
                                  GError **error) {
  return g_key_file_get_string(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                               key, error);
}
...
char *egg_desktop_file_get_locale_string(EggDesktopFile *desktop_file,
  ...
  return g_key_file_get_locale_string(
      desktop_file->key_file, EGG_DESKTOP_FILE_GROUP, key, locale, error);
}

gboolean egg_desktop_file_get_boolean(EggDesktopFile *desktop_file,
                                      const char *key, GError **error) {
  return g_key_file_get_boolean(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                                key, error);
}

double egg_desktop_file_get_numeric(EggDesktopFile *desktop_file,
                                    const char *key, GError **error) {
  return g_key_file_get_double(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                               key, error);
}

int egg_desktop_file_get_integer(EggDesktopFile *desktop_file, const char *key,
                                 GError **error) {
  return g_key_file_get_integer(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                                key, error);
}
...
char **egg_desktop_file_get_string_list(EggDesktopFile *desktop_file,
  ...
  return g_key_file_get_string_list(desktop_file->key_file,
                                    EGG_DESKTOP_FILE_GROUP, key, length, error);
}
...
gboolean egg_desktop_file_can_launch(EggDesktopFile *desktop_file,
  ...
  if (desktop_environment) {
    ...
    not_show_in = g_key_file_get_string_list(
        desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
...
static void append_quoted_word(GString *str, const char *s,
...
static char *parse_exec(EggDesktopFile *desktop_file, GSList **documents,
                        GError **error) {
  char *exec, *p, *command;
  gboolean escape, single_quot, double_quot;
  GString *gs;

  exec = g_key_file_get_string(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                               EGG_DESKTOP_FILE_KEY_EXEC, error);
  if (!exec)
    return NULL;
  ...
  if (g_key_file_has_key(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                         EGG_DESKTOP_FILE_KEY_TERMINAL, NULL)) {
    GError *terminal_error = NULL;
    gboolean use_terminal =
        g_key_file_get_boolean(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                               EGG_DESKTOP_FILE_KEY_TERMINAL, &terminal_error);
    if (terminal_error) {
      g_free(command);
...
static GSList *translate_document_list(EggDesktopFile *desktop_file,
                                       GSList *documents) {
...
static void free_document_list(GSList *documents) {
  GSList *d;
...
static gboolean parse_link(EggDesktopFile *desktop_file,
                           EggDesktopFile **app_desktop_file,
                           GSList **documents, GError **error) {
  char *url;
  GKeyFile *key_file;

  url = g_key_file_get_string(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                              EGG_DESKTOP_FILE_KEY_URL, error);
  if (!url)
    return FALSE;
...
static char *start_startup_notification(GdkDisplay *display,
                                        EggDesktopFile *desktop_file,
                                        const char *argv0, int screen,
                                        int workspace, guint32 launch_time) {
  ...
  if (g_key_file_has_key(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
    ...
    if (!g_key_file_get_boolean(desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                                EGG_DESKTOP_FILE_KEY_STARTUP_NOTIFY, NULL))
      return NULL;
    wmclass = NULL;
  } else {
  ...
  startup_id = g_strdup_printf("%s-%lu-%s-%s-%d_TIME%lu", g_get_prgname(),
                               (unsigned long)getpid(), g_get_host_name(),
                               argv0, sequence++, (unsigned long)launch_time);
  ...
  gdk_x11_display_broadcast_startup_message(
      display, "new", "ID", startup_id, "NAME", desktop_file->name, "SCREEN",
      screen_str, "BIN", argv0, "ICON", desktop_file->icon, "DESKTOP",
      workspace_str, "DESCRIPTION", description, "WMCLASS", wmclass, NULL);
...
static void end_startup_notification(GdkDisplay *display,
                                     const char *startup_id) {
  gdk_x11_display_broadcast_startup_message(display, "remove", "ID", startup_id,
                                            NULL);
}
...
static gboolean startup_notification_timeout(gpointer data) {
...
static void set_startup_notification_timeout(GdkDisplay *display,
                                             const char *startup_id) {
...
static GPtrArray *array_putenv(GPtrArray *env, char *variable) {
  guint i, keylen;
  ...
  if (!env) {
    ...
    for (i = 0; envp[i]; i++) {
      ...
      g_ptr_array_add(env,
                      g_strdup_printf("%s=%s", envp[i], value ? value : ""));
    }
    g_strfreev(envp);
  }
...
static gboolean egg_desktop_file_launchv(EggDesktopFile *desktop_file,
  ...
  while ((option = va_arg(args, EggDesktopFileLaunchOption))) {
    ...
    default:
      g_set_error(error, EGG_DESKTOP_FILE_ERROR,
                  ...
                  _("Unrecognized launch option: %d"), GPOINTER_TO_INT(option));
      success = FALSE;
      goto out;
    }
  }
  ...
  do {
...
#if GTK_CHECK_VERSION(2, 12, 0)
    startup_id = start_startup_notification(display, desktop_file, argv[0],
                                            screen_num, workspace, launch_time);
    if (startup_id) {
      char *startup_id_env =
          g_strdup_printf("DESKTOP_STARTUP_ID=%s", startup_id);
    ...
    current_success = g_spawn_async_with_pipes(
        directory, argv, env ? (char **)(env->pdata) : NULL, flags, setup_func,
        setup_data, ret_pid, ret_stdin, ret_stdout, ret_stderr, error);
    g_strfreev(argv);
...
gboolean egg_desktop_file_launch(EggDesktopFile *desktop_file,
  ...
  case EGG_DESKTOP_FILE_TYPE_APPLICATION:
    va_start(args, error);
    success = egg_desktop_file_launchv(desktop_file, documents, args, error);
    va_end(args);
    break;
  ...
  case EGG_DESKTOP_FILE_TYPE_LINK:
    ...
    va_start(args, error);
    success =
        egg_desktop_file_launchv(app_desktop_file, documents, args, error);
    va_end(args);
...
static void egg_set_desktop_file_internal(const char *desktop_file_path,
