 ...
 *
 * #BaconVideoWidget is a widget to play audio or video streams. It has a
 *GStreamer backend, and abstracts away the differences to provide a simple
 *interface to the functionality required by Xplayer. It handles all the
 *low-level audio and video work for Xplayer (or passes the work off to the
 *backend).
 **/
...
#define is_error(e, d, c)                                                      \
  (e->domain == GST_##d##_ERROR && e->code == GST_##d##_ERROR_##c)
...
G_DEFINE_TYPE_WITH_CODE(
    BaconVideoWidget, bacon_video_widget, GTK_CLUTTER_TYPE_EMBED,
...
static const gchar *video_props_str[4] = {"brightness", "contrast",
                                          "saturation", "hue"};
...
static gboolean bvw_set_playback_direction(BaconVideoWidget *bvw,
                                           gboolean forward);
...
static gboolean is_gtk_plug(GtkWidget *toplevel) {
#ifdef GDK_WINDOWING_X11
...
static const gchar *get_type_name(GType class_type, int type) {
  GEnumClass *eclass;
  GEnumValue *value;
...
static gchar **bvw_get_missing_plugins_foo(const GList *missing_plugins,
                                           MsgToStrFunc func) {
...
static gchar **bvw_get_missing_plugins_details(const GList *missing_plugins) {
  return bvw_get_missing_plugins_foo(
      missing_plugins, gst_missing_plugin_message_get_installer_detail);
}

static gchar **
bvw_get_missing_plugins_descriptions(const GList *missing_plugins) {
  return bvw_get_missing_plugins_foo(
      missing_plugins, gst_missing_plugin_message_get_description);
}

static void bvw_clear_missing_plugins_messages(BaconVideoWidget *bvw) {
  g_list_foreach(bvw->priv->missing_plugins, (GFunc)gst_mini_object_unref,
                 NULL);
  g_list_free(bvw->priv->missing_plugins);
  bvw->priv->missing_plugins = NULL;
}

static void bvw_check_if_video_decoder_is_missing(BaconVideoWidget *bvw) {
  GList *l;
...
static void set_display_pixel_aspect_ratio(GdkScreen *screen, int monitor,
                                           GValue *value) {
...
#define DELTA(idx) (ABS(ratio - ((gdouble)par[idx][0] / par[idx][1])))
  ...
  ratio = (gdouble)(gdk_screen_get_monitor_width_mm(screen, monitor) *
                    rect.height) /
  ...
  GST_DEBUG("Decided on index %d (%d/%d)", par_index, par[par_index][0],
            par[par_index][1]);
  gst_value_set_fraction(value, par[par_index][0], par[par_index][1]);
}

static void get_media_size(BaconVideoWidget *bvw, gint *width, gint *height) {
  if (bvw->priv->logo_mode) {
    ...
    if (bvw->priv->media_has_video) {
      ...
      if (bvw->priv->video_width == 0 || bvw->priv->video_height == 0) {
        ...
        num = 1;
        den = 1;
      } else if (!gst_video_calculate_display_ratio(
                     &num, &den, bvw->priv->video_width,
                     bvw->priv->video_height, movie_par_n, movie_par_d,
...
static void bacon_video_widget_realize(GtkWidget *widget) {
  ...
  g_signal_connect(G_OBJECT(gtk_widget_get_screen(widget)), "size-changed",
                   G_CALLBACK(size_changed_cb), bvw);

  /* setup the toplevel, ready to be resized */
  toplevel = gtk_widget_get_toplevel(widget);
  if (gtk_widget_is_toplevel(toplevel) &&
      gtk_widget_get_parent(widget) != toplevel && !is_gtk_plug(toplevel))
...
static void set_current_actor(BaconVideoWidget *bvw) {
  gboolean draw_logo;
  ...
  if (bvw->priv->logo_mode || draw_logo) {
    ...
    if (pixbuf != NULL) {
      ...
      ret = clutter_texture_set_from_rgb_data(
          CLUTTER_TEXTURE(bvw->priv->logo), gdk_pixbuf_get_pixels(pixbuf),
          gdk_pixbuf_get_has_alpha(pixbuf), gdk_pixbuf_get_width(pixbuf),
...
static gboolean bacon_video_widget_motion_notify(GtkWidget *widget,
  ...
  if (bvw->priv->navigation && !bvw->priv->logo_mode)
    gst_navigation_send_mouse_event(bvw->priv->navigation, "mouse-move", 0,
                                    event->x, event->y);
...
static gboolean bacon_video_widget_button_press(GtkWidget *widget,
  ...
  if (bvw->priv->navigation && !bvw->priv->logo_mode) {
    gst_navigation_send_mouse_event(bvw->priv->navigation, "mouse-button-press",
...
static gboolean bacon_video_widget_button_release(GtkWidget *widget,
                                                  GdkEventButton *event) {
  gboolean res = FALSE;
  BaconVideoWidget *bvw = BACON_VIDEO_WIDGET(widget);
...
static void bacon_video_widget_get_preferred_width(GtkWidget *widget,
                                                   gint *minimum,
                                                   gint *natural) {
  *minimum = *natural = 240;
}

static void bacon_video_widget_get_preferred_height(GtkWidget *widget,
                                                    gint *minimum,
                                                    gint *natural) {
  *minimum = *natural = 180;
}

static gboolean bvw_boolean_handled_accumulator(GSignalInvocationHint *ihint,
                                                GValue *return_accu,
...
static void bacon_video_widget_class_init(BaconVideoWidgetClass *klass) {
   ...
   *
   * Whether the logo should be displayed when no stream is loaded, or the
   *widget should take up no space.
   **/
  g_object_class_install_property(
      object_class, PROP_LOGO_MODE,
      g_param_spec_boolean(
          "logo-mode", "Logo mode?",
          "Whether the logo should be displayed when no stream is loaded.",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  /**
   * BaconVideoWidget:position:
   *
   * The current position in the stream, as a percentage between <code
   *class="literal">0</code> and <code class="literal">1</code>.
   **/
  g_object_class_install_property(
      object_class, PROP_POSITION,
      g_param_spec_double("position", "Position",
                          "The current position in the stream.", 0, 1.0, 0,
   ...
   **/
  g_object_class_install_property(
      object_class, PROP_STREAM_LENGTH,
   ...
   *
   * The current volume level, as a percentage between <code
   *class="literal">0</code> and <code class="literal">1</code>.
   **/
  g_object_class_install_property(
      object_class, PROP_VOLUME,
      g_param_spec_double("volume", "Volume", "The current volume level.", 0.0,
                          1.0, 0.0,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("user-agent", "User agent",
                          "The HTTP user agent string to use.", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   **/
  g_object_class_install_property(
      object_class, PROP_DOWNLOAD_FILENAME,
      g_param_spec_string("download-filename", "Download filename.",
                          "The filename of the fully downloaded stream.", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  /**
   * BaconVideoWidget:auto-resize:
   *
   * Whether to automatically resize the video widget to the video size when
   *loading a new video.
   **/
  g_object_class_install_property(
      object_class, PROP_AUTO_RESIZE,
      g_param_spec_boolean(
          "auto-resize", "Auto resize?",
          "Whether to automatically resize the video widget to the video size.",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   **/
  g_object_class_install_property(
      object_class, PROP_DEINTERLACING,
   ...
   **/
  g_object_class_install_property(
      object_class, PROP_HUE,
      g_param_spec_int("hue", "Hue", "The hue of the video display.", 0, 65535,
                       32768, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   **/
  g_object_class_install_property(
      object_class, PROP_AUDIO_OUTPUT_TYPE,
      g_param_spec_enum("audio-output-type", "Audio output type",
                        "The type of audio output to use.",
                        BVW_TYPE_AUDIO_OUTPUT_TYPE, BVW_AUDIO_SOUND_STEREO,
                        G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   **/
  g_object_class_install_property(
      object_class, PROP_AV_OFFSET,
      g_param_spec_int64(
          "av-offset", "Audio/Video offset",
   ...
   * @playback_stopped: %TRUE if playback has stopped due to the error, %FALSE
   *otherwise
   * @fatal: %TRUE if the error was fatal to playback, %FALSE otherwise
   *
   * Emitted when the backend wishes to asynchronously report an error. If
   *@fatal is %TRUE, playback of this stream cannot be restarted.
   **/
  bvw_signals[SIGNAL_ERROR] = g_signal_new(
      I_("error"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(BaconVideoWidgetClass, error), NULL, NULL,
   ...
   **/
  bvw_signals[SIGNAL_EOS] = g_signal_new(
      I_("eos"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(BaconVideoWidgetClass, eos), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  /**
   * BaconVideoWidget::got-metadata:
   *
   * Emitted when the widget has updated the metadata of the current stream.
   *This will typically happen just after opening a stream.
   *
   * Call bacon_video_widget_get_metadata() to query the updated metadata.
   **/
  bvw_signals[SIGNAL_GOT_METADATA] = g_signal_new(
      I_("got-metadata"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(BaconVideoWidgetClass, got_metadata), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
   ...
   **/
  bvw_signals[SIGNAL_REDIRECT] = g_signal_new(
      I_("got-redirect"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(BaconVideoWidgetClass, got_redirect), NULL, NULL,
   ...
   *
   * Query the new list of audio languages with
   *bacon_video_widget_get_languages().
   **/
  bvw_signals[SIGNAL_CHANNELS_CHANGE] = g_signal_new(
      I_("channels-change"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(BaconVideoWidgetClass, channels_change), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  /**
   * BaconVideoWidget::tick:
   * @current_time: the current position in the stream, in milliseconds since
   *the beginning of the stream
   * @stream_length: the length of the stream, in milliseconds
   * @current_position: the current position in the stream, as a percentage
   *between <code class="literal">0</code> and <code class="literal">1</code>
   * @seekable: %TRUE if the stream can be seeked, %FALSE otherwise
   *
   * Emitted every time an important time event happens, or at regular intervals
   *when playing a stream.
   **/
  bvw_signals[SIGNAL_TICK] = g_signal_new(
      I_("tick"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(BaconVideoWidgetClass, tick), NULL, NULL,
      g_cclosure_marshal_generic, G_TYPE_NONE, 4, G_TYPE_INT64, G_TYPE_INT64,
      G_TYPE_DOUBLE, G_TYPE_BOOLEAN);

  /**
   * BaconVideoWidget::buffering:
   * @percentage: the percentage of buffering completed, between <code
   *class="literal">0</code> and <code class="literal">1</code>
   *
   * Emitted regularly when a network stream is being buffered, to provide
   *status updates on the buffering progress.
   **/
  bvw_signals[SIGNAL_BUFFERING] = g_signal_new(
      I_("buffering"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(BaconVideoWidgetClass, buffering), NULL, NULL,
      g_cclosure_marshal_VOID__DOUBLE, G_TYPE_NONE, 1, G_TYPE_DOUBLE);

  /**
   * BaconVideoWidget::missing-plugins:
   * @details: a %NULL-terminated array of missing plugin details for use when
   *installing the plugins with libgimme-codec
   * @descriptions: a %NULL-terminated array of missing plugin descriptions for
   *display to the user
   * @playing: %TRUE if the stream could be played even without these plugins,
   *%FALSE otherwise
   *
   * Emitted when plugins required to play the current stream are not found.
   *This allows the application to request the user install them before
   *proceeding to try and play the stream again.
   *
   * Note that this signal is only available for the GStreamer backend.
   *
   * Return value: %TRUE if the signal was handled and some action was taken,
   *%FALSE otherwise
   **/
  bvw_signals[SIGNAL_MISSING_PLUGINS] = g_signal_new(
      I_("missing-plugins"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      0, /* signal is enough, we don't need a vfunc */
      bvw_boolean_handled_accumulator, NULL, g_cclosure_marshal_generic,
      G_TYPE_BOOLEAN, 3, G_TYPE_STRV, G_TYPE_STRV, G_TYPE_BOOLEAN);

  /**
   * BaconVideoWidget::download-buffering:
   * @percentage: the percentage of download buffering completed, between <code
   *class="literal">0</code> and <code class="literal">1</code>
   *
   * Emitted regularly when a network stream is being cached on disk, to provide
   *status updates on the buffering level of the stream.
   **/
  bvw_signals[SIGNAL_DOWNLOAD_BUFFERING] = g_signal_new(
      "download-buffering", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(BaconVideoWidgetClass, download_buffering), NULL, NULL,
      g_cclosure_marshal_VOID__DOUBLE, G_TYPE_NONE, 1, G_TYPE_DOUBLE);
}
...
static void bacon_video_widget_init(BaconVideoWidget *bvw) {
  ...
  bvw->priv = priv = G_TYPE_INSTANCE_GET_PRIVATE(bvw, BACON_TYPE_VIDEO_WIDGET,
  ...
  priv->tag_update_queue =
      g_async_queue_new_full((GDestroyNotify)update_tags_delayed_data_destroy);
...
static void bvw_update_stream_info(BaconVideoWidget *bvw) {
...
static void bvw_handle_application_message(BaconVideoWidget *bvw,
                                           GstMessage *msg) {
...
static gboolean bvw_do_navigation_query(BaconVideoWidget *bvw,
                                        GstQuery *query) {
...
static void mount_cb(GObject *obj, GAsyncResult *res, gpointer user_data) {
  ...
  if (!ret)
    GST_DEBUG("Mounting location '%s' failed: %s", GST_STR_NULL(uri),
              error->message);
  else
    GST_DEBUG("Failed to set '%s' back to playing: %s", GST_STR_NULL(uri),
              error->message);

  /* create a fake GStreamer error so we get a nice warning message */
  err = g_error_new_literal(GST_RESOURCE_ERROR, GST_RESOURCE_ERROR_OPEN_READ,
                            error->message);
...
static void bvw_handle_element_message(BaconVideoWidget *bvw, GstMessage *msg) {
  const GstStructure *structure;
  ...
  if (strcmp(type_name, "redirect") == 0) {
    ...
    g_file_mount_enclosing_volume(file, G_MOUNT_MOUNT_NONE, mount_op,
                                  bvw->priv->mount_cancellable, mount_cb, bvw);
...
static gboolean bvw_signal_eos_delayed(gpointer user_data) {
...
static void bvw_reconfigure_tick_timeout(BaconVideoWidget *bvw, guint msecs) {
...
static gboolean bvw_emit_missing_plugins_signal(BaconVideoWidget *bvw,
                                                gboolean prerolled) {
  ...
  g_signal_emit(bvw, bvw_signals[SIGNAL_MISSING_PLUGINS], 0, details,
                descriptions, prerolled, &handled);
...
static void bvw_auth_reply_cb(GMountOperation *op, GMountOperationResult result,
...
static int bvw_get_http_error_code(GstMessage *err_msg) {
  GError *err = NULL;
  gchar *dbg = NULL;
  int code = -1;
  ...
  if (err == NULL || dbg == NULL)
    goto done;
  if (!is_error(err, RESOURCE, READ) && !is_error(err, RESOURCE, OPEN_READ))
    goto done;
...
static gboolean bvw_check_missing_auth(BaconVideoWidget *bvw,
                                       GstMessage *err_msg) {
  GtkWidget *toplevel;
  ...
  if (g_object_class_find_property(G_OBJECT_GET_CLASS(err_msg->src),
                                   "user-id") == NULL) {
    GST_DEBUG(
        "HTTP error is 401, but don't have \"user-id\" property, exiting");
    return FALSE;
  }

  GST_DEBUG("Trying to get auth for location '%s'",
            GST_STR_NULL(bvw->priv->mrl));
  ...
  klass->ask_password(
      bvw->priv->auth_dialog, _("Password requested for RTSP server"),
...
static gboolean bvw_check_missing_plugins_error(BaconVideoWidget *bvw,
                                                GstMessage *err_msg) {
...
/* returns TRUE if the error/signal has been handled and should be ignored */
static gboolean bvw_check_missing_plugins_on_preroll(BaconVideoWidget *bvw) {
  if (bvw->priv->missing_plugins == NULL) {
    GST_DEBUG("no missing-plugin messages");
    return FALSE;
  }
...
static void update_orientation_from_video(BaconVideoWidget *bvw) {
  ...
  ret = gst_tag_list_get_string_index(
      bvw->priv->tagcache, GST_TAG_IMAGE_ORIENTATION, 0, &orientation_str);
  ...
  angle = rotation * 90.0;
  xplayer_aspect_frame_set_rotation(XPLAYER_ASPECT_FRAME(bvw->priv->frame),
                                    angle);
}

static void bvw_update_tags(BaconVideoWidget *bvw, GstTagList *tag_list,
                            const gchar *type) {
  ...
  result =
      gst_tag_list_merge(bvw->priv->tagcache, tag_list, GST_TAG_MERGE_REPLACE);
...
static gboolean bvw_update_tags_dispatcher(BaconVideoWidget *self) {
  UpdateTagsDelayedData *data;

  /* If we take the queue's lock for the entire function call, we can use it to
   * protect tag_update_id too */
  g_async_queue_lock(self->priv->tag_update_queue);

  while ((data = g_async_queue_try_pop_unlocked(
              self->priv->tag_update_queue)) != NULL) {
...
static void bvw_update_tags_delayed(BaconVideoWidget *bvw, GstTagList *tags,
                                    const gchar *type) {
  ...
  if (bvw->priv->tag_update_id == 0)
    bvw->priv->tag_update_id =
        g_idle_add((GSourceFunc)bvw_update_tags_dispatcher, bvw);

  g_async_queue_unlock(bvw->priv->tag_update_queue);
}

static void video_tags_changed_cb(GstElement *playbin2, gint stream_id,
                                  gpointer user_data) {
  BaconVideoWidget *bvw = (BaconVideoWidget *)user_data;
  GstTagList *tags = NULL;
  gint current_stream_id = 0;

  g_object_get(G_OBJECT(bvw->priv->play), "current-video", &current_stream_id,
               NULL);
  ...
  g_signal_emit_by_name(G_OBJECT(bvw->priv->play), "get-video-tags", stream_id,
                        &tags);
...
static void audio_tags_changed_cb(GstElement *playbin2, gint stream_id,
                                  gpointer user_data) {
  BaconVideoWidget *bvw = (BaconVideoWidget *)user_data;
  GstTagList *tags = NULL;
  gint current_stream_id = 0;

  g_object_get(G_OBJECT(bvw->priv->play), "current-audio", &current_stream_id,
               NULL);
  ...
  g_signal_emit_by_name(G_OBJECT(bvw->priv->play), "get-audio-tags", stream_id,
                        &tags);
...
static void text_tags_changed_cb(GstElement *playbin2, gint stream_id,
                                 gpointer user_data) {
  BaconVideoWidget *bvw = (BaconVideoWidget *)user_data;
  GstTagList *tags = NULL;
  gint current_stream_id = 0;
  ...
  g_signal_emit_by_name(G_OBJECT(bvw->priv->play), "get-text-tags", stream_id,
                        &tags);
...
static gboolean bvw_download_buffering_done(BaconVideoWidget *bvw) {
  ...
  if (bvw->priv->buffering_left * BUFFERING_LEFT_RATIO <
...
static void bvw_handle_buffering_message(GstMessage *message,
                                         BaconVideoWidget *bvw) {
  ...
  g_signal_emit(bvw, bvw_signals[SIGNAL_BUFFERING], 0,
...
static inline void bvw_get_navigation_if_available(BaconVideoWidget *bvw) {
  GstElement *nav;
  nav = gst_bin_get_by_interface(GST_BIN(bvw->priv->play), GST_TYPE_NAVIGATION);
...
static void bvw_bus_message_cb(GstBus *bus, GstMessage *message,
                               BaconVideoWidget *bvw) {
  ...
  case GST_MESSAGE_ERROR: {
    ...
    if (!bvw_check_missing_plugins_error(bvw, message) &&
      ...
      g_signal_emit(bvw, bvw_signals[SIGNAL_ERROR], 0, error->message, TRUE);
  ...
  case GST_MESSAGE_DURATION_CHANGED: {
    ...
    if (gst_element_query_duration(bvw->priv->play, GST_FORMAT_TIME, &len) &&
      ...
      GST_DEBUG(
          "got new stream length (through duration message) %" G_GINT64_FORMAT,
...
static void got_time_tick(GstElement *play, gint64 time_nanos,
                          BaconVideoWidget *bvw) {
  gboolean seekable;
  ...
  /*
    GST_DEBUG ("current time: %" GST_TIME_FORMAT ", stream length: %"
    GST_TIME_FORMAT ", seekable: %s", GST_TIME_ARGS (bvw->priv->current_time *
    GST_MSECOND), GST_TIME_ARGS (bvw->priv->stream_length * GST_MSECOND),
        (seekable) ? "TRUE" : "FALSE");
  */

  g_signal_emit(bvw, bvw_signals[SIGNAL_TICK], 0, bvw->priv->current_time,
                bvw->priv->stream_length, bvw->priv->current_position,
                seekable);
}

static void bvw_set_user_agent_on_element(BaconVideoWidget *bvw,
                                          GstElement *element) {
...
static void bvw_set_auth_on_element(BaconVideoWidget *bvw,
                                    GstElement *element) {
  if (g_object_class_find_property(G_OBJECT_GET_CLASS(element), "user-id") ==
      NULL)
    return;
  ...
  g_object_set(element, "user-id", bvw->priv->user_id, "user-pw",
...
static void bvw_set_http_proxy_on_element(BaconVideoWidget *bvw,
                                          GstElement *element) {
  GSettings *settings;
  ...
  g_object_set(element, "proxy-id", user_id, "proxy-pw", user_pw, NULL);
  g_free(user_pw);
  g_free(user_id);
...
static void bvw_set_proxy_on_element(BaconVideoWidget *bvw,
                                     GstElement *element) {
  GSettings *settings;
  GDesktopProxyMode mode;

  if (g_object_class_find_property(G_OBJECT_GET_CLASS(element), "proxy") ==
      NULL)
    return;
...
static void bvw_set_referrer_on_element(BaconVideoWidget *bvw,
                                        GstElement *element) {
  BaconVideoWidgetPrivate *priv = bvw->priv;
  ...
  GST_DEBUG("Setting HTTP referrer to '%s'",
            priv->referrer ? priv->referrer : "none");
  ...
  if (priv->referrer != NULL) {
    gst_structure_set(extra_headers, "Referer" /* not a typo! */, G_TYPE_STRING,
                      priv->referrer, NULL);
  } else {
    gst_structure_remove_field(extra_headers, "Referer" /* not a typo! */);
  }
...
static void playbin_source_setup_cb(GstElement *playbin, GstElement *source,
                                    BaconVideoWidget *bvw) {
...
static void playbin_deep_notify_cb(GstObject *gstobject, GstObject *prop_object,
                                   GParamSpec *prop, BaconVideoWidget *bvw) {
...
static gboolean bvw_query_buffering_timeout(BaconVideoWidget *bvw) {
  ...
  if (gst_element_query(element, query)) {
    ...
    gst_query_parse_buffering_stats(query, NULL, NULL, NULL,
    ...
    if (bvw_download_buffering_done(bvw) != FALSE &&
      ...
      GST_DEBUG(
          "Starting playback because the download buffer is filled enough");
      bacon_video_widget_play(bvw, NULL);
    }
...
static void caps_set(GObject *obj, GParamSpec *pspec, BaconVideoWidget *bvw) {
...
static void parse_stream_info(BaconVideoWidget *bvw) {
  ...
  g_object_get(G_OBJECT(bvw->priv->play), "n-audio", &n_audio, "n-video",
               &n_video, NULL);
...
static void playbin_stream_changed_cb(GstElement *obj, gpointer data) {
...
static void bacon_video_widget_set_property(GObject *object, guint property_id,
  ...
  case PROP_BRIGHTNESS:
    bacon_video_widget_set_video_property(bvw, BVW_VIDEO_BRIGHTNESS,
                                          g_value_get_int(value));
    break;
  case PROP_CONTRAST:
    bacon_video_widget_set_video_property(bvw, BVW_VIDEO_CONTRAST,
                                          g_value_get_int(value));
    break;
  case PROP_SATURATION:
    bacon_video_widget_set_video_property(bvw, BVW_VIDEO_SATURATION,
                                          g_value_get_int(value));
    break;
  case PROP_HUE:
    bacon_video_widget_set_video_property(bvw, BVW_VIDEO_HUE,
                                          g_value_get_int(value));
    break;
...
static void bacon_video_widget_get_property(GObject *object, guint property_id,
  ...
  case PROP_DEINTERLACING:
    ...
    break;
  case PROP_BRIGHTNESS:
    g_value_set_int(value, bacon_video_widget_get_video_property(
                               bvw, BVW_VIDEO_BRIGHTNESS));
    break;
  case PROP_CONTRAST:
    g_value_set_int(
        value, bacon_video_widget_get_video_property(bvw, BVW_VIDEO_CONTRAST));
    break;
  case PROP_SATURATION:
    g_value_set_int(value, bacon_video_widget_get_video_property(
                               bvw, BVW_VIDEO_SATURATION));
    break;
  case PROP_HUE:
    g_value_set_int(value,
                    bacon_video_widget_get_video_property(bvw, BVW_VIDEO_HUE));
    break;
  case PROP_AUDIO_OUTPUT_TYPE:
 ...
 *
 * If the widget is not playing, <code class="literal">-2</code> will be
 *returned. If no subtitles are being used, <code class="literal">-1</code> is
 *returned.
 *
 ...
 *
 * Sets the subtitle index for @bvw. If @subtitle is <code
 *class="literal">-1</code>, no subtitles will be used.
 **/
void bacon_video_widget_set_subtitle(BaconVideoWidget *bvw, int subtitle) {
  ...
  if (flags & GST_PLAY_FLAG_TEXT) {
    ...
    g_signal_emit_by_name(G_OBJECT(bvw->priv->play), "get-text-tags", subtitle,
                          &tags);
...
static GList *get_lang_list_for_type(BaconVideoWidget *bvw,
 ...
 * @bvw: a #BaconVideoWidget
 *
 * Returns a list of subtitle tags, each in the form <literal>TEXT
 *<replaceable>x</replaceable></literal>, where <replaceable>x</replaceable> is
 *the subtitle index.
 *
 * Return value: a #GList of subtitle tags, or %NULL; free each element with
 *g_free() and the list with g_list_free()
 **/
 ...
 *the language index.
 *
 * Return value: a #GList of audio language tags, or %NULL; free each element
 *with g_free() and the list with g_list_free()
 **/
 ...
 *
 * If the widget is not playing, or the default language is in use, <code
 ...
 *
 * Sets the audio language index for @bvw. If @language is <code
 *class="literal">-1</code>, the default language will be used.
 **/
void bacon_video_widget_set_language(BaconVideoWidget *bvw, int language) {
  ...
  g_signal_emit_by_name(G_OBJECT(bvw->priv->play), "get-audio-tags", language,
                        &tags);
 ...
 * @bvw: a #BaconVideoWidget
 * @deinterlace: %TRUE if videos should be automatically deinterlaced, %FALSE
 *otherwise
 *
...
static void set_audio_filter(BaconVideoWidget *bvw) {
  gint channels;
  GstCaps *caps, *res;
  GstPad *pad, *peer_pad;
...
void bacon_video_widget_show_osd(BaconVideoWidget *bvw, const char *icon_name,
                                 const char *message) {
  g_return_if_fail(BACON_IS_VIDEO_WIDGET(bvw));

  bacon_video_osd_actor_set_content(BACON_VIDEO_OSD_ACTOR(bvw->priv->osd),
                                    icon_name, message);
...
static GError *bvw_error_from_gst_error(BaconVideoWidget *bvw,
  ...
  if (is_error(e, RESOURCE, NOT_FOUND) || is_error(e, RESOURCE, OPEN_READ)) {
    if (g_str_has_prefix(bvw->priv->mrl, "dvd:")) {
      ret = g_error_new_literal(
          BVW_ERROR, BVW_ERROR_INVALID_DEVICE,
  ...
  if (http_error_code == 403) {
    ret = g_error_new_literal(
        BVW_ERROR, BVW_ERROR_FILE_PERMISSION,
  ...
  if (is_error(e, RESOURCE, OPEN_READ) || is_error(e, RESOURCE, READ)) {
  ...
  if (is_error(e, CORE, MISSING_PLUGIN) ||
      is_error(e, STREAM, CODEC_NOT_FOUND) || is_error(e, STREAM, WRONG_TYPE) ||
    ...
    if (bvw->priv->missing_plugins != NULL) {
      ...
      ret = g_error_new_literal(
          BVW_ERROR, BVW_ERROR_CODEC_NOT_HANDLED,
  ...
  if (is_error(e, STREAM, FAILED) && src_typename &&
      strncmp(src_typename, "GstTypeFind", 11) == 0) {
    ret = g_error_new_literal(BVW_ERROR, BVW_ERROR_READ_ERROR,
                              _("This file cannot be played over the network. "
 ...
 **/
void bacon_video_widget_open(BaconVideoWidget *bvw, const char *mrl) {
  GFile *file;
...
static gboolean bacon_video_widget_seek_time_no_lock(BaconVideoWidget *bvw,
  ...
  gst_element_seek(bvw->priv->play, bvw->priv->rate, GST_FORMAT_TIME,
                   GST_SEEK_FLAG_FLUSH | flag, GST_SEEK_TYPE_SET,
                   _time * GST_MSECOND, GST_SEEK_TYPE_NONE,
                   GST_CLOCK_TIME_NONE);
 ...
 * @accurate: whether to use accurate seek, an accurate seek might be slower for
 *some formats (see GStreamer docs)
 * @error: a #GError, or %NULL
 *
 * Seeks the currently-playing stream to the absolute position @time, in
 *milliseconds.
 *
 * Return value: %TRUE on success, %FALSE otherwise
 **/
gboolean bacon_video_widget_seek_time(BaconVideoWidget *bvw, gint64 _time,
                                      gboolean accurate, GError **error) {
  ...
  if (bvw->priv->seek_req_time == GST_CLOCK_TIME_NONE ||
      cur_time > bvw->priv->seek_req_time + SEEK_TIMEOUT || accurate) {
    bvw->priv->seek_time = -1;
 ...
 * Return value: %TRUE on success, %FALSE otherwise
 **/
gboolean bacon_video_widget_seek(BaconVideoWidget *bvw, double position,
                                 GError **error) {
 ...
 * @error: a #GError, or %NULL
 *
 * Step one frame forward, if @forward is %TRUE, or backwards, if @forward is
 *%FALSE
 *
 * Return value: %TRUE on success, %FALSE otherwise
 **/
gboolean bacon_video_widget_step(BaconVideoWidget *bvw, gboolean forward,
                                 GError **error) {
  GstEvent *event;
  gboolean retval;
...
static void bvw_stop_play_pipeline(BaconVideoWidget *bvw) {
  ...
  if (cur_state > GST_STATE_READY) {
    ...
    while ((msg = gst_bus_pop_filtered(bvw->priv->bus,
  ...
  xplayer_aspect_frame_set_internal_rotation(
      XPLAYER_ASPECT_FRAME(bvw->priv->frame), 0.0);
  GST_DEBUG("stopped");
}
 ...
 * @bvw: a #BaconVideoWidget
 *
 * Stops playing the current stream and resets to the first position in the
 *stream.
 **/
...
static void bvw_do_navigation_command(BaconVideoWidget *bvw,
...
void bacon_video_widget_set_text_subtitle(BaconVideoWidget *bvw,
  ...
  if (subtitle_uri == NULL && bvw->priv->subtitle_uri == NULL)
    return;
  ...
  case BVW_DVD_PREV_TITLE: {
    ...
    if (gst_element_query_position(bvw->priv->play, fmt, &val)) {
      ...
      gst_element_seek(bvw->priv->play, FORWARD_RATE, fmt, GST_SEEK_FLAG_FLUSH,
                       GST_SEEK_TYPE_SET, val, GST_SEEK_TYPE_NONE,
                       G_GINT64_CONSTANT(0));
      bvw->priv->rate = FORWARD_RATE;
    } else {
 ...
 **/
void bacon_video_widget_set_logo(BaconVideoWidget *bvw, const gchar *name) {
  GtkIconTheme *theme;
  GError *error = NULL;
  ...
  bvw->priv->logo_pixbuf =
      gtk_icon_theme_load_icon(theme, name, LOGO_SIZE, 0, &error);

  if (error) {
    g_warning("An error occurred trying to open logo %s: %s", name,
              error->message);
    g_error_free(error);
    return;
  }
...
void bacon_video_widget_pause(BaconVideoWidget *bvw) {
  ...
  if (bvw->priv->is_live != FALSE && ret != GST_STATE_CHANGE_NO_PREROLL &&
      ret != GST_STATE_CHANGE_SUCCESS && state > GST_STATE_READY) {
...
void bacon_video_widget_set_subtitle_font(BaconVideoWidget *bvw,
  ...
  if (!g_object_class_find_property(G_OBJECT_GET_CLASS(bvw->priv->play),
                                    "subtitle-font-desc"))
    return;
 ...
 * @bvw: a #BaconVideoWidget
 * @volume: the new volume level, as a percentage between <code
 *class="literal">0</code> and <code class="literal">1</code>
 *
 * Sets the volume level of the stream as a percentage between <code
 *class="literal">0</code> and <code class="literal">1</code>.
 *
 ...
 * @bvw: a #BaconVideoWidget
 *
 * Returns the current volume level, as a percentage between <code
 *class="literal">0</code> and <code class="literal">1</code>.
 *
 * Return value: the volume as a percentage between <code
 *class="literal">0</code> and <code class="literal">1</code>
 **/
 ...
 * displayed. If @ratio is set to <code class="literal">0</code>, the highest
 *ratio possible will be chosen.
 **/
...
BvwZoomMode bacon_video_widget_get_zoom(BaconVideoWidget *bvw) {
  ...
  expand =
      xplayer_aspect_frame_get_expand(XPLAYER_ASPECT_FRAME(bvw->priv->frame));
...
void bacon_video_widget_set_rotation(BaconVideoWidget *bvw,
  ...
  GST_DEBUG("Rotating to %s (%f degrees) from %s",
            get_type_name(BVW_TYPE_ROTATION, rotation), rotation * 90.0,
...
bvw_get_color_balance_channel(GstColorBalance *color_balance,
  ...
  for (channels = gst_color_balance_list_channels(color_balance);
       channels != NULL; channels = channels->next) {
 ...
 *
 * Returns the given property of the video display, such as its brightness or
 *saturation.
 *
 * It is returned as a percentage in the full range of integer values; from
 *<code class="literal">0</code> to <code class="literal">65535</code>
 *(inclusive), where <code class="literal">32768</code> is the default.
 *
 * Return value: the property's value, in the range <code
 *class="literal">0</code> to <code class="literal">65535</code>
 **/
int bacon_video_widget_get_video_property(BaconVideoWidget *bvw,
  ...
  found_channel =
      bvw_get_color_balance_channel(GST_COLOR_BALANCE(bvw->priv->play), type);
  cur = gst_color_balance_get_value(GST_COLOR_BALANCE(bvw->priv->play),
                                    found_channel);

  GST_DEBUG("channel %s: cur=%d, min=%d, max=%d", found_channel->label, cur,
            found_channel->min_value, found_channel->max_value);

  ret = floor(
      0.5 + ((double)cur - found_channel->min_value) * 65535 /
...
void bacon_video_widget_set_next_angle(BaconVideoWidget *bvw) {
  ...
  g_object_get(G_OBJECT(bvw->priv->play), "current-video", &current_video,
               "n-video", &n_video, NULL);

  if (n_video <= 1) {
    GST_DEBUG("Not setting next video stream, we have %d video streams",
              n_video);
    return;
  }
 ...
 * @bvw: a #BaconVideoWidget
 * @type: the type of property
 * @value: the property's value, in the range <code class="literal">0</code> to
 *<code class="literal">65535</code>
 *
 * Sets the given property of the video display, such as its brightness or
 *saturation.
 *
...
void bacon_video_widget_set_video_property(BaconVideoWidget *bvw,
  ...
  i_value = floor(
      0.5 +
      value * ((double)found_channel->max_value - found_channel->min_value) /
          65535 +
      found_channel->min_value);

  GST_DEBUG("channel %s: set to %d/65535", found_channel->label, value);

  gst_color_balance_set_value(GST_COLOR_BALANCE(bvw->priv->play), found_channel,
                              i_value);

  GST_DEBUG("channel %s: val=%d, min=%d, max=%d", found_channel->label, i_value,
            found_channel->min_value, found_channel->max_value);
 ...
 *
 * Return value: time since the beginning of the stream, in milliseconds, or
 ...
 *
 * Return value: the stream length, in milliseconds, or <code
...
static gint bvw_get_current_stream_num(BaconVideoWidget *bvw,
...
static GstTagList *bvw_get_tags_of_current_stream(BaconVideoWidget *bvw,
                                                  const gchar *stream_type) {
  GstTagList *tags = NULL;
  gint stream_num = -1;
...
static GstCaps *bvw_get_caps_of_current_stream(BaconVideoWidget *bvw,
                                               const gchar *stream_type) {
  GstCaps *caps = NULL;
  gint stream_num = -1;
  GstPad *current;
...
static gboolean audio_caps_have_LFE(GstStructure *s) {
  guint64 mask;
  int channels;
...
static void bacon_video_widget_get_metadata_string(BaconVideoWidget *bvw,
  ...
  case BVW_INFO_TITLE:
    if (bvw->priv->tagcache != NULL) {
      res = gst_tag_list_get_string_index(bvw->priv->tagcache, GST_TAG_TITLE, 0,
                                          &string);
    }
    break;
  case BVW_INFO_ARTIST:
    if (bvw->priv->tagcache != NULL) {
      res = gst_tag_list_get_string_index(bvw->priv->tagcache, GST_TAG_ARTIST,
                                          0, &string);
    }
    break;
  case BVW_INFO_YEAR:
    if (bvw->priv->tagcache != NULL) {
      ...
      if ((res = gst_tag_list_get_date(bvw->priv->tagcache, GST_TAG_DATE,
                                       &date))) {
        string = g_strdup_printf("%d", g_date_get_year(date));
        g_date_free(date);
      } else if ((res = gst_tag_list_get_date_time(
                      bvw->priv->tagcache, GST_TAG_DATE_TIME, &datetime))) {
  ...
  case BVW_INFO_COMMENT:
    if (bvw->priv->tagcache != NULL) {
      res = gst_tag_list_get_string_index(bvw->priv->tagcache, GST_TAG_COMMENT,
                                          0, &string);
  ...
  case BVW_INFO_ALBUM:
    if (bvw->priv->tagcache != NULL) {
      res = gst_tag_list_get_string_index(bvw->priv->tagcache, GST_TAG_ALBUM, 0,
                                          &string);
    }
    break;
  ...
  case BVW_INFO_VIDEO_CODEC: {
    ...
    if (!res && bvw->priv->tagcache != NULL) {
      res = gst_tag_list_get_string(bvw->priv->tagcache, GST_TAG_VIDEO_CODEC,
                                    &string);
    }
    break;
  }
  case BVW_INFO_AUDIO_CODEC: {
    ...
    /* if that didn't work, try the aggregated tags */
    if (!res && bvw->priv->tagcache != NULL) {
      res = gst_tag_list_get_string(bvw->priv->tagcache, GST_TAG_AUDIO_CODEC,
                                    &string);
    }
    break;
  }
...
static void bacon_video_widget_get_metadata_int(BaconVideoWidget *bvw,
                                                BvwMetadataType type,
                                                GValue *value) {
  int integer = 0;
  ...
  case BVW_INFO_TRACK_NUMBER:
    if (bvw->priv->tagcache == NULL)
      break;
    if (!gst_tag_list_get_uint(bvw->priv->tagcache, GST_TAG_TRACK_NUMBER,
                               (guint *)&integer))
      integer = 0;
    break;
  case BVW_INFO_DIMENSION_X:
...
static void bacon_video_widget_get_metadata_bool(BaconVideoWidget *bvw,
                                                 BvwMetadataType type,
                                                 GValue *value) {
  ...
  GST_DEBUG("%s = %s", get_type_name(BVW_TYPE_METADATA_TYPE, type),
            (boolean) ? "yes" : "no");
 ...
 *
 * Return value: a #GOptionGroup giving command-line options for
 *#BaconVideoWidget
 **/
...
GQuark bacon_video_widget_error_quark(void) {
  static GQuark q; /* 0 */
...
static gboolean bvw_set_playback_direction(BaconVideoWidget *bvw,
  ...
  if (forward == FALSE) {
    ...
    if (gst_element_query_position(bvw->priv->play, GST_FORMAT_TIME, &cur)) {
      GST_DEBUG("Setting playback direction to reverse at %" G_GINT64_FORMAT "",
                cur);
      event = gst_event_new_seek(REVERSE_RATE, GST_FORMAT_TIME,
    ...
    cur = 0;
    if (gst_element_query_position(bvw->priv->play, GST_FORMAT_TIME, &cur)) {
      GST_DEBUG("Setting playback direction to forward at %" G_GINT64_FORMAT "",
                cur);
      event = gst_event_new_seek(FORWARD_RATE, GST_FORMAT_TIME,
                                 GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_ACCURATE,
                                 GST_SEEK_TYPE_SET, cur, GST_SEEK_TYPE_SET,
                                 G_GINT64_CONSTANT(0));
      if (gst_element_send_event(bvw->priv->play, event) == FALSE) {
...
static gboolean bacon_video_widget_initable_init(GInitable *initable,
  ...
  bvw->priv->audio_pitchcontrol =
      element_make_or_warn("scaletempo", "scaletempo");
#ifdef HAVE_CLUTTER_GST_3
  ...
  if (!bvw->priv->play || !bvw->priv->audio_pitchcontrol || !video_sink ||
      !audio_sink) {
    if (video_sink)
    ...
    g_set_error_literal(
        error, BVW_ERROR, BVW_ERROR_PLUGIN_LOAD,
  ...
  bvw->priv->sig_bus_async = g_signal_connect(
      bvw->priv->bus, "message", G_CALLBACK(bvw_bus_message_cb), bvw);
  ...
  clutter_actor_set_layout_manager(
      bvw->priv->stage, clutter_bin_layout_new(CLUTTER_BIN_ALIGNMENT_FILL,
...
#ifdef HAVE_CLUTTER_GST_3
  bvw->priv->texture = g_object_new(
      CLUTTER_TYPE_ACTOR, "content",
      g_object_new(CLUTTER_GST_TYPE_CONTENT, "sink", video_sink, NULL), "name",
      "texture", NULL);
#else
  bvw->priv->texture =
  ...
  xplayer_aspect_frame_set_child(XPLAYER_ASPECT_FRAME(bvw->priv->logo_frame),
                                 bvw->priv->logo);
  clutter_actor_add_child(CLUTTER_ACTOR(bvw->priv->stage),
                          bvw->priv->logo_frame);
  clutter_actor_hide(CLUTTER_ACTOR(bvw->priv->logo_frame));
  ...
  xplayer_aspect_frame_set_child(XPLAYER_ASPECT_FRAME(bvw->priv->frame),
                                 bvw->priv->texture);
  ...
  clutter_actor_set_pivot_point(bvw->priv->osd, -OSD_MARGIN,
                                -OSD_MARGIN); /* FIXME RTL */
  clutter_actor_set_size(bvw->priv->osd, OSD_SIZE, OSD_SIZE);
  clutter_actor_add_child(bvw->priv->stage, bvw->priv->osd);
  clutter_actor_set_child_above_sibling(bvw->priv->stage, bvw->priv->osd,
                                        bvw->priv->frame);
  ...
  gst_bin_add_many(GST_BIN(audio_bin), bvw->priv->audio_capsfilter, audio_sink,
                   NULL);
  gst_element_link_many(bvw->priv->audio_capsfilter, audio_sink, NULL);
  ...
  g_object_set(bvw->priv->play, "audio-filter", bvw->priv->audio_pitchcontrol,
               NULL);
 ...
 *
 * Return value: a new #BaconVideoWidget, or %NULL; destroy with
 *gtk_widget_destroy()
 **/
...
gboolean bacon_video_widget_set_rate(BaconVideoWidget *bvw, gfloat new_rate) {
  ...
  if (gst_element_query_position(bvw->priv->play, GST_FORMAT_TIME, &cur)) {
    ...
    event = gst_event_new_seek(
        new_rate, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_ACCURATE,
        GST_SEEK_TYPE_SET, cur, GST_SEEK_TYPE_SET, GST_CLOCK_TIME_NONE);
    if (gst_element_send_event(bvw->priv->play, event) == FALSE) {
