...
/* Debug log message handler: discards debug messages unless Xplayer is run with
 * XPLAYER_DEBUG=1.
...
static void debug_handler(const char *log_domain, GLogLevelFlags log_level,
                          const char *message, GSettings *settings) {
  static int debug = -1;
...
static void set_rtl_icon_name(Xplayer *xplayer, const char *widget_name,
...
static void app_init(Xplayer *xplayer, char **argv) {
  GtkSettings *gtk_settings;
  char *sidebar_pageid;

  if (gtk_clutter_init(NULL, NULL) != CLUTTER_INIT_SUCCESS)
    g_warning(
        "gtk-clutter failed to initialise, expect problems from here on.");
  ...
  xplayer->win =
      GTK_WIDGET(gtk_builder_get_object(xplayer->xml, "xplayer_main_window"));
  ...
  xplayer->seek =
      GTK_WIDGET(gtk_builder_get_object(xplayer->xml, "tmw_seek_hscale"));
  xplayer->seekadj = gtk_range_get_adjustment(GTK_RANGE(xplayer->seek));
  xplayer->volume =
      GTK_WIDGET(gtk_builder_get_object(xplayer->xml, "tmw_volume_button"));
  xplayer->seek_lock = FALSE;
  xplayer->fs = xplayer_fullscreen_new(GTK_WINDOW(xplayer->win));
  gtk_scale_button_set_adjustment(
      GTK_SCALE_BUTTON(xplayer->fs->volume),
  ...
  xplayer->prefs_xml =
      xplayer_interface_load("preferences.ui", TRUE, NULL, xplayer);
  ...
  } else if (optionstate.filenames != NULL &&
             xplayer_action_open_files(xplayer, optionstate.filenames)) {
  ...
  gtk_window_set_application(GTK_WINDOW(xplayer->win),
                             GTK_APPLICATION(xplayer));
}

static void app_startup(GApplication *application, Xplayer *xplayer) {
...
static int app_command_line(GApplication *app,
...
int main(int argc, char **argv) {
  ...
  if (XInitThreads() == 0) {
    ...
    xplayer_action_error_and_exit(
        _("Could not initialize the thread-safe libraries."),
        _("Verify your system installation. Xplayer will now exit."), NULL);
  }
#endif
  ...
  xplayer = g_object_new(XPLAYER_TYPE_OBJECT, "application-id", "org.x.Player",
                         "flags", G_APPLICATION_HANDLES_COMMAND_LINE, NULL);
  xplayer->settings = g_settings_new(XPLAYER_GSETTINGS_SCHEMA);

  g_signal_connect(G_OBJECT(xplayer), "startup", G_CALLBACK(app_startup),
                   xplayer);
