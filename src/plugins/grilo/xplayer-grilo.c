...
#define grl_operation_options_set_resolution_flags                             \
  grl_operation_options_set_flags
#define grl_registry_load_all_plugins(a, b, c)                                 \
  grl_registry_load_all_plugins(a, c)
...
#define XPLAYER_TYPE_GRILO_PLUGIN (xplayer_grilo_plugin_get_type())
#define XPLAYER_GRILO_PLUGIN(o)                                                \
  (G_TYPE_CHECK_INSTANCE_CAST((o), XPLAYER_TYPE_GRILO_PLUGIN,                  \
                              XplayerGriloPlugin))
#define XPLAYER_GRILO_PLUGIN_CLASS(k)                                          \
  (G_TYPE_CHECK_CLASS_CAST((k), XPLAYER_TYPE_GRILO_PLUGIN,                     \
                           XplayerGriloPluginClass))
...
#define XPLAYER_GRILO_PLUGIN_GET_CLASS(o)                                      \
  (G_TYPE_INSTANCE_GET_CLASS((o), XPLAYER_TYPE_GRILO_PLUGIN,                   \
                             XplayerGriloPluginClass))
...
const gchar *BLACKLIST_SOURCES[] = {"grl-filesystem", "grl-bookmarks",
                                    "grl-shoutcast", "grl-flickr", NULL};

typedef enum { ICON_BOX = 0, ICON_VIDEO } IconType;
...
XPLAYER_PLUGIN_REGISTER(XPLAYER_TYPE_GRILO_PLUGIN, XplayerGriloPlugin,
                        xplayer_grilo_plugin)
...
static void play(XplayerGriloPlugin *self, GrlSource *source, GrlMedia *media,
                 gboolean resolve_url);

static gchar *get_secondary_text(GrlMedia *media) {
  const char *artist;
  int duration;
...
static GList *browse_keys(void) {
  static GList *_browse_keys = NULL;

  if (_browse_keys == NULL) {
    _browse_keys = grl_metadata_key_list_new(
        GRL_METADATA_KEY_ARTIST, GRL_METADATA_KEY_AUTHOR,
        GRL_METADATA_KEY_DURATION, GRL_METADATA_KEY_THUMBNAIL,
        GRL_METADATA_KEY_URL, GRL_METADATA_KEY_TITLE, NULL);
  }

  return _browse_keys;
}

static GList *search_keys(void) {
  static GList *_search_keys = NULL;

  if (_search_keys == NULL) {
    _search_keys = grl_metadata_key_list_new(
        GRL_METADATA_KEY_ARTIST, GRL_METADATA_KEY_AUTHOR,
        GRL_METADATA_KEY_DURATION, GRL_METADATA_KEY_THUMBNAIL,
        GRL_METADATA_KEY_URL, GRL_METADATA_KEY_TITLE, NULL);
  }
...
static GdkPixbuf *load_icon(XplayerGriloPlugin *self, IconType icon_type,
                            gint thumb_size) {
  GdkScreen *screen;
  GtkIconTheme *theme;

  const gchar *icon_name[] = {GTK_STOCK_DIRECTORY, "video-x-generic"};

  static GdkPixbuf *pixbuf[G_N_ELEMENTS(icon_name)] = {NULL};

  if (pixbuf[icon_type] == NULL) {
    screen =
        gtk_window_get_screen(xplayer_get_main_window(self->priv->xplayer));
    theme = gtk_icon_theme_get_for_screen(screen);
    pixbuf[icon_type] = gtk_icon_theme_load_icon(theme, icon_name[icon_type],
                                                 thumb_size, 0, NULL);
  }
...
static GdkPixbuf *get_icon(XplayerGriloPlugin *self, GrlMedia *media,
                           gint thumb_size) {
...
static void get_stream_thumbnail_cb(GObject *source_object, GAsyncResult *res,
                                    gpointer user_data) {
  ...
  if (stream != NULL) {
    thumbnail = gdk_pixbuf_new_from_stream_at_scale(
        G_INPUT_STREAM(stream), thumb_data->thumb_size, thumb_data->thumb_size,
        TRUE, NULL, NULL);
    g_object_unref(stream);
  }

  gtk_tree_model_get_iter(
      thumb_data->xplayer_grilo->priv->search_results_model, &iter,
      gtk_tree_row_reference_get_path(thumb_data->reference));

  if (thumbnail) {
    gtk_list_store_set(
        GTK_LIST_STORE(thumb_data->xplayer_grilo->priv->search_results_model),
        &iter, GD_MAIN_COLUMN_ICON, thumbnail, -1);
    /* Cache it */
    g_hash_table_insert(thumb_data->xplayer_grilo->priv->cache_thumbnails,
                        g_file_get_uri(thumb_data->file), thumbnail);
  }
...
static void set_thumbnail_async(XplayerGriloPlugin *self, GrlMedia *media,
                                GtkTreePath *path, gint thumb_size) {
  ...
  if (url_thumb != NULL) {
    /* Check cache */
    thumbnail = g_hash_table_lookup(self->priv->cache_thumbnails, url_thumb);
    if (thumbnail == NULL) {
      ...
      thumb_data->reference =
          gtk_tree_row_reference_new(self->priv->search_results_model, path);
    ...
    /* Keep the icon */
    gtk_tree_model_get_iter(self->priv->search_results_model, &iter, path);
    gtk_list_store_set(GTK_LIST_STORE(self->priv->search_results_model), &iter,
                       MODEL_RESULTS_IS_PRETHUMBNAIL, FALSE, -1);
  }
}

static gboolean update_search_thumbnails_idle(XplayerGriloPlugin *self) {
  ...
  if (gtk_icon_view_get_visible_range(
          GTK_ICON_VIEW(self->priv->search_results_view), &start_path,
          &end_path)) {
    while (gtk_tree_path_compare(start_path, end_path) <= 0) {

      if (gtk_tree_model_get_iter(self->priv->search_results_model, &iter,
                                  start_path) == FALSE)
        break;

      gtk_tree_model_get(self->priv->search_results_model, &iter,
                         MODEL_RESULTS_CONTENT, &media,
                         MODEL_RESULTS_IS_PRETHUMBNAIL, &is_prethumbnail, -1);
      if (is_prethumbnail) {
        set_thumbnail_async(self, media, start_path, THUMB_SEARCH_SIZE);
        gtk_list_store_set(GTK_LIST_STORE(self->priv->search_results_model),
                           &iter, MODEL_RESULTS_IS_PRETHUMBNAIL, FALSE, -1);
      }
...
static void update_search_thumbnails(XplayerGriloPlugin *self) {
  g_idle_add((GSourceFunc)update_search_thumbnails_idle, self);
}

static void show_sources(XplayerGriloPlugin *self) {
  ...
  sources =
      grl_registry_get_sources_by_operations(registry, GRL_OP_BROWSE, FALSE);
  ...
  for (source = sources; source; source = source->next) {
    ...
    gtk_tree_store_append(GTK_TREE_STORE(self->priv->browser_model), &iter,
                          NULL);
    gtk_tree_store_set(GTK_TREE_STORE(self->priv->browser_model), &iter,
                       MODEL_RESULTS_SOURCE, source->data,
                       MODEL_RESULTS_CONTENT, NULL, GD_MAIN_COLUMN_PRIMARY_TEXT,
                       name, GD_MAIN_COLUMN_ICON, icon,
                       MODEL_RESULTS_IS_PRETHUMBNAIL, FALSE, -1);
...
static void browse_cb(GrlSource *source, guint browse_id, GrlMedia *media,
                      guint remaining, gpointer user_data,
                      const GError *error) {
  GtkTreeIter iter;
  GdkPixbuf *thumbnail;
  BrowseUserData *bud;
  ...
  if (error != NULL &&
      g_error_matches(error, GRL_CORE_ERROR,
  ...
  if (media != NULL) {
    ...
    gtk_tree_model_get_iter(self->priv->browser_model, &parent,
                            gtk_tree_row_reference_get_path(bud->ref_parent));
    gtk_tree_model_get(self->priv->browser_model, &parent,
                       MODEL_RESULTS_REMAINING, &remaining_expected, -1);
    remaining_expected--;
    gtk_tree_store_set(GTK_TREE_STORE(self->priv->browser_model), &parent,
                       MODEL_RESULTS_REMAINING, &remaining_expected, -1);
    /* Filter images */
    if (grl_media_is_image(media) || grl_media_is_audio(media)) {
      g_object_unref(media);
      goto out;
    }
    ...
    gtk_tree_store_append(GTK_TREE_STORE(self->priv->browser_model), &iter,
                          &parent);
    gtk_tree_store_set(GTK_TREE_STORE(self->priv->browser_model), &iter,
                       MODEL_RESULTS_SOURCE, source, MODEL_RESULTS_CONTENT,
                       media, GD_MAIN_COLUMN_ICON, thumbnail,
                       MODEL_RESULTS_IS_PRETHUMBNAIL, TRUE,
                       GD_MAIN_COLUMN_PRIMARY_TEXT, grl_media_get_title(media),
                       GD_MAIN_COLUMN_SECONDARY_TEXT, secondary, -1);
...
static void browse(XplayerGriloPlugin *self, GtkTreePath *path,
                   GrlSource *source, GrlMedia *container, gint page) {
  if (source != NULL) {
    BrowseUserData *bud;
    ...
    bud->ref_parent =
        gtk_tree_row_reference_new(self->priv->browser_model, path);
    grl_source_browse(source, container, browse_keys(), default_options,
                      browse_cb, bud);
...
static void resolve_url_cb(GrlSource *source, guint op_id, GrlMedia *media,
...
static void play(XplayerGriloPlugin *self, GrlSource *source, GrlMedia *media,
  ...
  if (resolve_url && grl_source_supported_operations(source) & GRL_OP_RESOLVE) {
    const GList *slow_keys;

    slow_keys = grl_source_slow_keys(source);

    if (g_list_find((GList *)slow_keys,
                    GINT_TO_POINTER(GRL_METADATA_KEY_URL)) != NULL) {
      GList *url_keys;
      GrlOperationOptions *resolve_options;

      resolve_options = grl_operation_options_new(NULL);
      grl_operation_options_set_resolution_flags(resolve_options,
                                                 RESOLVE_FLAGS);

      url_keys = grl_metadata_key_list_new(GRL_METADATA_KEY_URL, NULL);
      grl_source_resolve(source, media, url_keys, resolve_options,
                         resolve_url_cb, self);
...
static void search_cb(GrlSource *source, guint search_id, GrlMedia *media,
                      guint remaining, gpointer user_data,
                      const GError *error) {
  GdkPixbuf *thumbnail;
  GtkWindow *window;
  ...
  if (media != NULL) {
    ...
    gtk_list_store_insert_with_values(
        GTK_LIST_STORE(self->priv->search_results_model), NULL, -1,
        MODEL_RESULTS_SOURCE, source, MODEL_RESULTS_CONTENT, media,
        GD_MAIN_COLUMN_ICON, thumbnail, MODEL_RESULTS_IS_PRETHUMBNAIL, TRUE,
        GD_MAIN_COLUMN_PRIMARY_TEXT, grl_media_get_title(media),
        GD_MAIN_COLUMN_SECONDARY_TEXT, secondary, -1);
...
static GrlOperationOptions *get_search_options(XplayerGriloPlugin *self) {
  GrlOperationOptions *default_options;
  GrlOperationOptions *supported_options;

  default_options = grl_operation_options_new(NULL);
  grl_operation_options_set_resolution_flags(default_options, BROWSE_FLAGS);
  grl_operation_options_set_skip(default_options,
                                 self->priv->search_page * PAGE_SIZE);
  grl_operation_options_set_count(default_options, PAGE_SIZE);
...
static void search_more(XplayerGriloPlugin *self) {
  ...
  if (self->priv->search_source != NULL) {
    self->priv->search_id =
        grl_source_search(self->priv->search_source, self->priv->search_text,
                          search_keys(), search_options, search_cb, self);
  } else {
    self->priv->search_id =
        grl_multiple_search(NULL, self->priv->search_text, search_keys(),
                            search_options, search_cb, self);
  }
...
static void search(XplayerGriloPlugin *self, GrlSource *source,
                   const gchar *text) {
...
static void search_entry_activate_cb(GtkEntry *entry,
  ...
  id = xplayer_search_entry_get_selected_id(
      XPLAYER_SEARCH_ENTRY(self->priv->search_entry));
  g_return_if_fail(id != NULL);
  registry = grl_registry_get_default();
  source = grl_registry_lookup_source(registry, id);
  g_return_if_fail(source != NULL);

  text = xplayer_search_entry_get_text(
      XPLAYER_SEARCH_ENTRY(self->priv->search_entry));
  g_return_if_fail(text != NULL);
  search(self, source, text);
}

static void browser_activated_cb(GtkTreeView *tree_view, GtkTreePath *path,
  ...
  gtk_tree_model_get(model, &iter, MODEL_RESULTS_SOURCE, &source,
                     MODEL_RESULTS_CONTENT, &content, MODEL_RESULTS_PAGE, &page,
                     MODEL_RESULTS_REMAINING, &remaining, -1);
  ...
  if (remaining == 0) {
    gtk_tree_store_set(GTK_TREE_STORE(model), &iter, MODEL_RESULTS_PAGE, ++page,
                       MODEL_RESULTS_REMAINING, PAGE_SIZE, -1);
...
static void search_entry_source_changed_cb(GObject *object, GParamSpec *pspec,
                                           XplayerGriloPlugin *self) {
...
static void search_activated_cb(GtkIconView *icon_view, GtkTreePath *path,
                                gpointer user_data) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  GrlSource *source;
  ...
  gtk_tree_model_get_iter(model, &iter, path);
  gtk_tree_model_get(model, &iter, MODEL_RESULTS_SOURCE, &source,
                     MODEL_RESULTS_CONTENT, &content, -1);
...
static gboolean source_is_blacklisted(GrlSource *source) {
...
static void source_added_cb(GrlRegistry *registry, GrlSource *source,
                            gpointer user_data) {
  const gchar *name;
  XplayerGriloPlugin *self;
  GrlSupportedOps ops;
  ...
  if (ops & GRL_OP_BROWSE) {
    ...
    gtk_tree_store_insert_with_values(
        GTK_TREE_STORE(self->priv->browser_model), NULL, NULL, -1,
        MODEL_RESULTS_SOURCE, source, MODEL_RESULTS_CONTENT, NULL,
        GD_MAIN_COLUMN_PRIMARY_TEXT, name, GD_MAIN_COLUMN_ICON, icon,
        MODEL_RESULTS_IS_PRETHUMBNAIL, TRUE, -1);

    g_clear_object(&icon);
  }
  if (ops & GRL_OP_SEARCH) {
    /* FIXME:
     * Handle tracker/filesystem specifically, so that we have a "local" entry
     * here */
    xplayer_search_entry_add_source(
        XPLAYER_SEARCH_ENTRY(self->priv->search_entry),
        grl_source_get_id(source), name, 0); /* FIXME: Use correct priority */
  }
}

static gboolean remove_browse_result(GtkTreeModel *model, GtkTreePath *path,
                                     GtkTreeIter *iter, gpointer user_data) {
  GrlSource *removed_source = GRL_SOURCE(user_data);
  GrlSource *model_source;
  gboolean same_source;

  gtk_tree_model_get(model, iter, MODEL_RESULTS_SOURCE, &model_source, -1);
...
static void source_removed_cb(GrlRegistry *registry, GrlSource *source,
                              gpointer user_data) {
  GrlSupportedOps ops;
  XplayerGriloPlugin *self = XPLAYER_GRILO_PLUGIN(user_data);
  ...
  if (ops & GRL_OP_BROWSE) {
    gtk_tree_model_foreach(self->priv->browser_model, remove_browse_result,
                           source);
  }
  ...
  if (ops & GRL_OP_SEARCH) {
    ...
    xplayer_search_entry_remove_source(
        XPLAYER_SEARCH_ENTRY(self->priv->search_entry), id);
  }
}

static void load_grilo_plugins(XplayerGriloPlugin *self) {
  GrlRegistry *registry;
  GError *error = NULL;

  registry = grl_registry_get_default();

  g_signal_connect(registry, "source-added", G_CALLBACK(source_added_cb), self);
  g_signal_connect(registry, "source-removed", G_CALLBACK(source_removed_cb),
                   self);
...
static gboolean show_popup_menu(XplayerGriloPlugin *self, GtkWidget *view,
  ...
  gtk_tree_model_get(model, &iter, MODEL_RESULTS_SOURCE, &source,
                     MODEL_RESULTS_CONTENT, &(self->priv->selected_media), -1);
  ...
  action =
      gtk_action_group_get_action(self->priv->action_group, "add-to-playlist");
  gtk_action_set_sensitive(action, url != NULL);
  action =
      gtk_action_group_get_action(self->priv->action_group, "copy-location");
  gtk_action_set_sensitive(action, url != NULL);

  menu = gtk_ui_manager_get_widget(self->priv->ui_manager, "/grilo-popup");
  gtk_menu_shell_select_first(GTK_MENU_SHELL(menu), FALSE);
  gtk_menu_popup(GTK_MENU(menu), NULL, NULL, NULL, NULL, button, _time);
...
static gboolean popup_menu_cb(GtkWidget *view, XplayerGriloPlugin *self) {
  return show_popup_menu(self, view, NULL);
}

static gboolean context_button_pressed_cb(GtkWidget *view,
                                          GdkEventButton *event,
                                          XplayerGriloPlugin *self) {
...
static gboolean adjustment_over_limit(GtkAdjustment *adjustment) {
  if ((gtk_adjustment_get_value(adjustment) +
       gtk_adjustment_get_page_size(adjustment)) /
          gtk_adjustment_get_upper(adjustment) >
      SCROLL_GET_MORE_LIMIT) {
    return TRUE;
  } else {
    return FALSE;
  }
...
static void get_more_browse_results_cb(GtkAdjustment *adjustment,
                                       XplayerGriloPlugin *self) {
  GtkTreePath *start_path;
  GtkTreePath *end_path;
  ...
  /* Start to check from last visible element, and check if its parent can get
   * more elements */
  while (gtk_tree_path_compare(start_path, end_path) <= 0 &&
    ...
    if (gtk_tree_path_up(parent_path) == FALSE ||
        gtk_tree_model_get_iter(self->priv->browser_model, &iter,
                                parent_path) == FALSE) {
      gtk_tree_path_free(parent_path);
      goto continue_next;
    }
    gtk_tree_model_get(self->priv->browser_model, &iter, MODEL_RESULTS_SOURCE,
                       &source, MODEL_RESULTS_CONTENT, &container,
                       MODEL_RESULTS_PAGE, &page, MODEL_RESULTS_REMAINING,
                       &remaining, -1);
    /* Skip non-boxes (they can not be browsed) */
    if (container != NULL && grl_media_is_container(container) == FALSE) {
      goto free_elements;
    }
  ...
  continue_next:
    stop_processing =
        stop_processing || (gtk_tree_path_prev(end_path) == FALSE);
  }
...
static void setup_sidebar_browse(XplayerGriloPlugin *self,
                                 GtkBuilder *builder) {
  self->priv->browser_model = GTK_TREE_MODEL(
      gtk_builder_get_object(builder, "gw_browse_store_results"));
  self->priv->browser =
      GTK_WIDGET(gtk_builder_get_object(builder, "gw_browse"));

  g_signal_connect(self->priv->browser, "row-activated",
                   G_CALLBACK(browser_activated_cb), self);
  g_signal_connect(self->priv->browser, "popup-menu", G_CALLBACK(popup_menu_cb),
                   self);
  g_signal_connect(self->priv->browser, "button-press-event",
                   G_CALLBACK(context_button_pressed_cb), self);
  g_signal_connect(
      gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(self->priv->browser)),
      "value_changed", G_CALLBACK(get_more_browse_results_cb), self);

  xplayer_add_sidebar_page(
      self->priv->xplayer, "grilo-browse", _("Browse"),
      GTK_WIDGET(gtk_builder_get_object(builder, "gw_browse_window")));
}

static void setup_sidebar_search(XplayerGriloPlugin *self,
                                 GtkBuilder *builder) {
  ...
  self->priv->search_sources_list =
      GTK_WIDGET(gtk_builder_get_object(builder, "gw_search_select_source"));
  search_combo_box =
      GTK_WIDGET(gtk_builder_get_object(builder, "gw_combo_box"));
  search_results_window =
      GTK_WIDGET(gtk_builder_get_object(builder, "gw_search_results_window"));
  ...
  search_results_view_style =
      gtk_widget_get_style_context(self->priv->search_results_view);
  ...
  g_signal_connect(GTK_ICON_VIEW(self->priv->search_results_view),
                   "item-activated", G_CALLBACK(search_activated_cb), self);
  g_signal_connect(self->priv->search_results_view, "popup-menu",
                   G_CALLBACK(popup_menu_cb), self);
  g_signal_connect(self->priv->search_results_view, "button-press-event",
                   G_CALLBACK(context_button_pressed_cb), self);
  ...
  g_signal_connect(gtk_scrolled_window_get_vadjustment(
                   ...
                   "value_changed", G_CALLBACK(adjustment_value_changed_cb),
                   self);
  ...
  gtk_box_pack_end(GTK_BOX(search_combo_box), self->priv->search_entry, TRUE,
                   TRUE, 0);
  ...
  xplayer_add_sidebar_page(
      self->priv->xplayer, "grilo-search", _("Search"),
      GTK_WIDGET(gtk_builder_get_object(builder, "gw_search")));
}

static void add_to_pls_cb(GtkAction *action, XplayerGriloPlugin *self) {
  xplayer_add_to_playlist_and_play(
      self->priv->xplayer, grl_media_get_url(self->priv->selected_media),
      grl_media_get_title(self->priv->selected_media));
}

static void copy_location_cb(GtkAction *action, XplayerGriloPlugin *self) {
  GtkClipboard *clip;
  const gchar *url;
...
static void setup_menus(XplayerGriloPlugin *self, GtkBuilder *builder) {
  GtkAction *action;
  GError *error = NULL;
  ...
  gtk_action_group_add_action_with_accel(self->priv->action_group, action,
                                         NULL);
  ...
  gtk_ui_manager_insert_action_group(self->priv->ui_manager,
                                     self->priv->action_group, 1);
  gtk_ui_manager_add_ui_from_string(self->priv->ui_manager, GRILO_POPUP_MENU,
                                    -1, &error);
  if (error != NULL) {
...
static void setup_ui(XplayerGriloPlugin *self, GtkBuilder *builder) {
  setup_sidebar_browse(self, builder);
  setup_sidebar_search(self, builder);
  setup_menus(self, builder);
}

static void setup_config(XplayerGriloPlugin *self) {
  gchar *config_file;
  ...
  config_file = g_build_path(G_DIR_SEPARATOR_S, g_get_user_config_dir(),
                             g_get_prgname(), XPLAYER_GRILO_CONFIG_FILE, NULL);
...
static void impl_activate(PeasActivatable *plugin) {
  ...
  priv->cache_thumbnails =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);

  builder = xplayer_plugin_load_interface("grilo", "grilo.ui", TRUE,
                                          main_window, self);
...
static void impl_deactivate(PeasActivatable *plugin) {
  ...
  for (s = sources; s; s = g_list_next(s)) {
    grl_registry_unregister_source(registry, GRL_SOURCE(s->data), NULL);
  }
  g_list_free(sources);
