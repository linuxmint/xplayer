...
#define GALLERY_HEADER_HEIGHT                                                  \
  66 /* header height (in pixels) for the gallery                              \
      */
...
static void entry_parsed_cb(XplayerPlParser *parser, const char *uri,
                            GHashTable *metadata, char **new_url) {
  *new_url = g_strdup(uri);
}

static char *get_special_url(GFile *file) {
  ...
  g_signal_connect(parser, "entry-parsed", G_CALLBACK(entry_parsed_cb), &uri);
...
static gboolean is_special_uri(const char *uri) {
...
static void thumb_app_set_filename(ThumbApp *app) {
  GFile *file;
  char *uri;
...
static GstBusSyncReply error_handler(GstBus *bus, GstMessage *message,
                                     GstElement *play) {
...
static void thumb_app_cleanup(ThumbApp *app) {
  gst_element_set_state(app->play, GST_STATE_NULL);
  g_clear_object(&app->play);
}

static void thumb_app_set_error_handler(ThumbApp *app) {
  GstBus *bus;
...
static void check_cover_for_stream(ThumbApp *app, const char *signal_name) {
  GdkPixbuf *pixbuf;
...
static void thumb_app_check_for_cover(ThumbApp *app) {
...
static gboolean thumb_app_set_duration(ThumbApp *app) {
  gint64 len = -1;

  if (gst_element_query_duration(app->play, GST_FORMAT_TIME, &len) &&
      len != -1) {
...
static void assert_duration(ThumbApp *app) {
  if (app->duration != -1)
    return;
...
static gboolean thumb_app_get_has_video(ThumbApp *app) {
  guint n_video;
  g_object_get(app->play, "n-video", &n_video, NULL);
  return n_video > 0;
}

static gboolean thumb_app_start(ThumbApp *app) {
  GstBus *bus;
  ...
  while (terminate == FALSE) {
    ...
    message = gst_bus_timed_pop_filtered(bus, GST_CLOCK_TIME_NONE, events);
    ...
    case GST_MESSAGE_ASYNC_DONE:
      ...
      break;
    case GST_MESSAGE_ERROR:
      xplayer_gst_message_print(message, app->play,
                                "xplayer-video-thumbnailer-error");
      terminate = TRUE;
      break;
...
static void thumb_app_setup_play(ThumbApp *app) {
  GstElement *play;
  ...
  g_object_set(play, "audio-sink", audio_sink, "video-sink", video_sink,
               "flags", GST_PLAY_FLAG_VIDEO | GST_PLAY_FLAG_AUDIO, NULL);
...
static void thumb_app_seek(ThumbApp *app, gint64 _time) {
  gst_element_seek(app->play, 1.0, GST_FORMAT_TIME,
                   GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_KEY_UNIT,
                   GST_SEEK_TYPE_SET, _time * GST_MSECOND, GST_SEEK_TYPE_NONE,
                   GST_CLOCK_TIME_NONE);
...
static gboolean is_image_interesting(GdkPixbuf *pixbuf) {
...
static GdkPixbuf *scale_pixbuf(GdkPixbuf *pixbuf, int size, gboolean is_still) {
  GdkPixbuf *result;
  int width, height;
  ...
  if (size <= 256) {
    ...
    small =
        gdk_pixbuf_scale_simple(pixbuf, d_width, d_height, GDK_INTERP_BILINEAR);
    result = small;
  } else {
    if (size > 0)
...
static void save_pixbuf(GdkPixbuf *pixbuf, const char *path,
  ...
  if (jpeg_output == FALSE) {
    ...
    ret = gdk_pixbuf_save(with_holes, path, "png", &err,
                          ...
                          "tEXt::Thumb::Image::Height", a_height, NULL);
  } else {
  ...
  if (ret == FALSE) {
    if (err != NULL) {
      g_print("xplayer-video-thumbnailer couldn't write the thumbnail '%s' for "
              "video '%s': %s\n",
...
static GdkPixbuf *capture_frame_at_time(ThumbApp *app, gint64 milliseconds) {
  if (milliseconds != 0)
...
static GdkPixbuf *capture_interesting_frame(ThumbApp *app) {
  GdkPixbuf *pixbuf;
  guint current;
  const double frame_locations[] = {1.0 / 3.0, 2.0 / 3.0, 0.1, 0.9, 0.5};
...
static GdkPixbuf *cairo_surface_to_pixbuf(cairo_surface_t *surface) {
  ...
  return gdk_pixbuf_new_from_data(output, GDK_COLORSPACE_RGB, FALSE, 8, width,
                                  height, width * 3,
                                  (GdkPixbufDestroyNotify)g_free, NULL);
}

static GdkPixbuf *create_gallery(ThumbApp *app) {
  ...
  gint screenshot_width = 0, screenshot_height = 0, x_padding = 0,
       y_padding = 0;
  gfloat scale = 1.0;
  ...
  if (gallery == 0) {
    ...
    while (gallery % 3 != 0 && gallery % 4 != 0 && gallery % 5 != 0) {
      gallery++;
    }
  }
  ...
  /* Put a lower bound on the screenshot interval so we can't enter an infinite
   * loop below */
  if (screenshot_interval == 0)
    screenshot_interval = 1;

  PROGRESS_DEBUG(
      "Producing gallery of %u screenshots, taken at %" G_GINT64_FORMAT
      " millisecond intervals throughout a %" G_GINT64_FORMAT
      " millisecond-long stream.",
      gallery, screenshot_interval, stream_length);

  /* Calculate how to arrange the screenshots so we don't get ones orphaned on
   * the last row. At this point, only deal with arrangements of 3, 4 or 5
   * columns. */
  y = G_MAXUINT;
  for (x = 3; x <= 5; x++) {
  ...
  for (pos = screenshot_interval; pos <= stream_length;
       pos += screenshot_interval) {
    ...
    if (pixbuf == NULL) {
      ...
      PROGRESS_DEBUG("Created output pixbuf (%ux%u).",
                     gdk_pixbuf_get_width(pixbuf),
                     gdk_pixbuf_get_height(pixbuf));
    }

    /* Composite the screenshot into our gallery */
    gdk_pixbuf_composite(screenshot, pixbuf, x, y, output_size,
                         scale * screenshot_height, (gdouble)x, (gdouble)y,
                         scale, scale, GDK_INTERP_BILINEAR, 255);
    g_object_unref(screenshot);

    PROGRESS_DEBUG("Composited screenshot from %" G_GINT64_FORMAT
                   " milliseconds (address %u) at (%u,%u).",
                   pos, GPOINTER_TO_UINT(screenshot), x, y);

    /* We print progress in the range 10% (MIN_PROGRESS) to 50% (MAX_PROGRESS -
     * MIN_PROGRESS) / 2.0 */
    PRINT_PROGRESS(MIN_PROGRESS +
                   (current_row * columns + current_column) *
                       (((MAX_PROGRESS - MIN_PROGRESS) / gallery) / 2.0));
  ...
  /* Load the pixbuf into a Cairo surface and overlay the text. The height is
   * the height of the gallery plus the necessary height for 3 lines of header
   * (at ~18px each), plus some extra padding. */
  surface = cairo_image_surface_create(
      CAIRO_FORMAT_RGB24, gdk_pixbuf_get_width(pixbuf),
  ...
  gdk_cairo_set_source_pixbuf(cr, pixbuf, 0.0,
                              GALLERY_HEADER_HEIGHT + y_padding);
  cairo_rectangle(cr, 0.0, GALLERY_HEADER_HEIGHT + y_padding,
  ...
  /* Translators: The first string is "Filename" (as translated); the second is
     an actual filename. The third string is "Resolution" (as translated); the
     fourth and fifth are screenshot height and width, respectively. The sixth
     string is "Duration" (as translated); the seventh is the movie duration in
     words. */
  header_text = g_markup_printf_escaped(
      _("<b>%s</b>: %s\n<b>%s</b>: %d\303\227%d\n<b>%s</b>: %s"), _("Filename"),
      filename, _("Resolution"), screenshot_width, screenshot_height,
      _("Duration"), duration_text);
  ...
  for (pos = screenshot_interval; pos <= stream_length;
    ...
    /* Display the timestamp in the bottom-right corner of the current
     * screenshot */
    cairo_move_to(cr, x - layout_width - 0.02 * output_size,
                  y - layout_height - 0.02 * scale * screenshot_height);

    /* We have to stroke the text so it's visible against screenshots of the
    ...
    /* We print progress in the range 50% (MAX_PROGRESS - MIN_PROGRESS) / 2.0)
     * to 90% (MAX_PROGRESS) */
    PRINT_PROGRESS(MIN_PROGRESS + (MAX_PROGRESS - MIN_PROGRESS) / 2.0 +
                   (current_row * columns + current_column) *
                       (((MAX_PROGRESS - MIN_PROGRESS) / gallery) / 2.0));
...
static const GOptionEntry entries[] = {
    {"jpeg", 'j', 0, G_OPTION_ARG_NONE, &jpeg_output,
     "Output the thumbnail as a JPEG instead of PNG", NULL},
    {"size", 's', 0, G_OPTION_ARG_INT, &output_size,
     "Size of the thumbnail in pixels (with --gallery sets the size of "
     "individual screenshots)",
     NULL},
    {"raw", 'r', 0, G_OPTION_ARG_NONE, &raw_output,
     "Output the raw picture of the video without scaling or adding borders",
     NULL},
    {"no-limit", 'l', G_OPTION_FLAG_REVERSE, G_OPTION_ARG_NONE, &time_limit,
     "Don't limit the thumbnailing time to 30 seconds", NULL},
    {"verbose", 'v', 0, G_OPTION_ARG_NONE, &verbose, "Output debug information",
     NULL},
    {"time", 't', 0, G_OPTION_ARG_INT64, &second_index,
     "Choose this time (in seconds) as the thumbnail (can't be used with "
     "--gallery)",
     NULL},
    {"g-fatal-warnings", 0, 0, G_OPTION_ARG_NONE, &g_fatal_warnings,
     "Make all warnings fatal", NULL},
    {"gallery", 'g', 0, G_OPTION_ARG_INT, &gallery,
     "Output a gallery of the given number (0 is default) of screenshots "
     "(can't be used with --time)",
     NULL},
    {"print-progress", 'p', 0, G_OPTION_ARG_NONE, &print_progress,
     "Only print progress updates (can't be used with --verbose)", NULL},
...
int main(int argc, char *argv[]) {
  ...
  if (thumb_app_get_has_video(&app) == FALSE) {
    PROGRESS_DEBUG(
        "xplayer-video-thumbnailer couldn't find a video track in '%s'\n",
        input);
    exit(1);
  }
